...
export default class CloudflareAPIClient {
     ...
     *
     * @see https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/secrets/methods/bulk_update/
     *
     * @param {string} workerName - Worker name
     ...
     * @throws {CloudflareApiError} When Cloudflare rejects the mutation or the created version cannot be identified uniquely
     */
    async createWorkerSecretVersion(workerName, operations, options) {
        ...
        const operationTag = `kixx-secret-${ randomUUID() }`;
        const url = new URL(
            `${ BASE_URL }/accounts/${ this.#accountId }/workers/scripts/${ workerName }/secrets-bulk`,
        );
        const method = 'PATCH';
        const headers = {
        ...
        // The endpoint's response is secret metadata, not the created version.
        // A unique authored tag provides causal lookup without selecting latest.
        ...
        return await this.#findCreatedSecretVersion(workerName, operationTag);
    }
    ...
    async #findCreatedSecretVersion(workerName, operationTag) {
        const matches = [];

        for (let page = 1; ; page += 1) {
            const versions = await this.listWorkerVersions(workerName, {
                page,
                per_page: SECRET_VERSION_PAGE_SIZE,
            });

            versions.forEach((version) => {
                if (version.annotations?.['workers/tag'] === operationTag) {
                    matches.push(version);
                }
            });

            if (versions.length < SECRET_VERSION_PAGE_SIZE) {
                break;
            }
        }
        ...
        if (matches.length !== 1) {
            throw new CloudflareApiError(
                `Expected one Worker version tagged for this secret operation, found ${ matches.length }`,
                { status: 200, errors: [], method: 'GET', url: url.href },
            );
        }

        const [ version ] = matches;
        assertNonEmptyString(version.id, 'Cloudflare secret version response requires an id');
        assertNonEmptyString(version.created_on, 'Cloudflare secret version response requires created_on');

        return { versionId: version.id, createdAt: version.created_on };
    }
