 ...
 *     exist, or standalone creation would force a deployment.
 */
export async function prepareWorkerVersion(args) {
    ...
    if (state) {
        await verifySourceVersion({ apiClient, workerName, environment, versionId: state.versionId });
    }
...
export async function createPreparedWorkerVersion(args) {
    ...
    const nextState = {
        ...
        secretNames: prepared.secretNames,
    };
...
function validateSourceState(args) {
    ...
    const recordedNames = new Set(state.secretNames ?? []);
    const missingNames = secretNames.filter((name) => !recordedNames.has(name));

    if (missingNames.length > 0) {
        throw new UsageError(
            `Worker version ${ state.versionId } has no recorded value for declared ` +
            `secret${ missingNames.length === 1 ? '' : 's' }: ${ missingNames.join(', ') }. ` +
            'Set every declared secret before creating a Worker version.',
        );
    }
}
...
async function verifySourceVersion(args) {
    ...
    try {
        await apiClient.getWorkerVersion(workerName, versionId);
    } catch (error) {
        if (error.status === 404) {
            throw new UsageError(
