...
export default class PublishingAPIClient {
     ...
     *
     * The server's If-Match token is the quoted current release id. It is built
     * here from the release id rather than copied from a prior GET's ETag header,
     * because intermediaries may weaken, rewrite, or drop that header.
     * @param {string} buildId - Build pointer id
     * @param {string} releaseId - Release to assign
     * @param {Object} options - Assignment options
     * @param {string} [options.expectedReleaseId] - Release currently assigned to the build
     * @param {boolean} [options.expectUnassigned] - Require a never-assigned build
    ...
    async assignBuild(buildId, releaseId, options) {
        const {
            expectedReleaseId,
            expectUnassigned = false,
            reason = 'publish',
        } = options ?? {};
        const hasExpectedReleaseId = !isUndefined(expectedReleaseId);

        assertNonEmptyString(buildId);
        assertNonEmptyString(releaseId);
        assert(
            hasExpectedReleaseId !== expectUnassigned,
            'assignBuild() requires exactly one of expectedReleaseId or expectUnassigned',
        );
        if (hasExpectedReleaseId) {
            assertNonEmptyString(expectedReleaseId);
        }

        const headers = hasExpectedReleaseId
            ? { 'if-match': `"${ expectedReleaseId }"` }
            : { 'if-none-match': '*' };
        const response = await this.#request(`builds/${ encodeURIComponent(buildId) }`, {
            method: 'PUT',
            headers: {
