...
/**
 * Reads an environment's pair of dotenv files into plain objects of name to
 * string value. Parsing is deliberately minimal: no inline comments, no
 * variable expansion, no multi-line values, so a value always reads at
 * Cloudflare exactly as it reads in the file.
 *
 * The two files are read together rather than one at a time because the pair
 * is the unit that carries the meaning. Secrecy is expressed by which file a
 * value is written in — `.env.<environment>` is committed, and
 * `.env.<environment>.secrets` is not — so a caller holding only one of them
 * cannot tell a plain value from a secret.
 * @module env-file
 */
