...
export default async function releaseToCloudflare(args) {
    ...
    try {
        ...
        if (prepared.outcome === 'skipped') {
            ...
            // Read the pointer back before the deploy that will serve it.
            // Comparing the assignment identity as well as the Release catches
            // a concurrent write that landed on the same Release by a
            // different act, which the Release id alone cannot distinguish.
            ...
            if (staged.assignmentId !== assignment.assignmentId) {
                throw new Error(
                    `Staged build ${ buildId } resolved to assignment ${ staged.assignmentId }, ` +
                    `not ${ assignment.assignmentId }. Something reassigned the pointer after ` +
                    'this release staged it.',
                );
            }
        }
    } catch (cause) {
