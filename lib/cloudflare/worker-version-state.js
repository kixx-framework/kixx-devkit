...
import { isBoolean, isNonEmptyString, isPlainObject, isString } from 'kixx-assert';
import UsageError from '../usage-error.js';
import defaultFileSystem from '../file-system.js';

const STRING_OR_NULL_FIELDS = [ 'modulesHash', 'bindingsHash', 'configHash' ];
 ...
 * @property {string[]} [secretNames] - Sorted names present in this version; values are never recorded.
 *
 * Durable Object lifecycle is deliberately absent. The `exports` map is
 * declarative and Cloudflare reports which namespaces exist, so there is
 * nothing here to keep in sync and nothing to drift.
 */
...
// A check on known fields, not a whitelist. An unrecognized key — including one
// written by an older version of this tool — is ignored rather than rejected,
// and every field stays optional so a partial file degrades into "everything
// changed" rather than into an error.
function validateState(state, filepath) {
    ...
    if (state.secretNames !== undefined) {
        validateSecretNames(state.secretNames, filepath);
    }
}

function validateSecretNames(secretNames, filepath) {
    if (!Array.isArray(secretNames)) {
        throw new UsageError(`Expected "secretNames" to be an array in state file: ${ filepath }`);
    }
    ...
    secretNames.forEach((name) => {
        if (!isNonEmptyString(name)) {
            throw new UsageError(`Expected every "secretNames" entry to be a non-empty string in state file: ${ filepath }`);
        }
        if (seen.has(name)) {
            throw new UsageError(`Duplicate secret name "${ name }" in state file: ${ filepath }`);
        }
        if (previousName !== null && name < previousName) {
            throw new UsageError(`Expected "secretNames" to be sorted in state file: ${ filepath }`);
        }
