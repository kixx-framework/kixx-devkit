...
/**
 * Reports which dotted key paths do not resolve to non-empty strings.
 * @param {Object} source - Configuration object to inspect.
 * @param {string[]} keyPaths - Dotted key paths expected to contain non-empty strings.
 * @returns {string[]} The invalid keyPaths, in the given order.
 */
export function findMissingNonEmptyStringKeys(source, keyPaths) {
    return (keyPaths ?? []).filter((keyPath) => {
        return !isNonEmptyString(getValueAtPath(source, keyPath));
    });
}
