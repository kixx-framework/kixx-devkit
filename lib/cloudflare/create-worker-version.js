...
import { readDeclaredSecretNames, readEnvValues } from '../env-file.js';
...
import { compareSecretNames, readSecretBindingNames } from './worker-secret-names.js';
...
const REQUIRED_STATE_FIELDS = [ 'workerName', 'versionId' ];
 ...
 * @property {'prepared'|'skipped'|'created'|'resources-resolved'} outcome - What happened.
 ...
 * @property {string[]} undeclaredSecretNames - Secrets on the source version that `example.env.secrets` does not
 *     declare. Versions built from it do not inherit them. Empty when there is no source version.
 */
 ...
 * @param {boolean} [args.allowForcedDeployment=false] - Allow creation to provision a Durable Object namespace.
 * @param {() => string} [args.generateUniqueId] - Collision-resistant build-id component generator.
 ...
 *     exist, the source version is absent or stale, a declared secret is not
 *     set on the source version, or standalone creation would force a deployment.
 */
export async function prepareWorkerVersion(args) {
    const {
        projectDirectory,
        environment,
        cloudflareConfig,
        apiClient,
        force = false,
        allowForcedDeployment = false,
        bundleModules = defaultBundleModules,
        fileSystem = defaultFileSystem,
        now = () => new Date(),
        generateUniqueId = () => randomUUID(),
    } = args ?? {};
    ...
    const { provisionedClasses } = readWorkerRecord(worker, workerName);
    const state = await readWorkerVersionState({ projectDirectory, environment, fileSystem });
    const secretNames = await readDeclaredSecretNames({ projectDirectory, fileSystem });

    validateSourceState({ state, workerName, environment, secretNames });

    let undeclaredSecretNames = [];
    if (state) {
        const sourceVersion = await verifySourceVersion({
            apiClient,
            workerName,
            environment,
        ...
        undeclaredSecretNames = assertDeclaredSecretsSet({
            sourceVersion,
            versionId: state.versionId,
            secretNames,
            environment,
    ...
    const envars = await readEnvValues({
        filepath: path.join(projectDirectory, `.env.${ environment }`),
        fileSystem,
    });
    ...
    const bindings = buildWorkerBindings({
        ...
        envars,
        secretNames,
        secretVersionId: state?.versionId,
    });
    ...
    const shouldUpload = force || !state || changes.modules || changes.bindings || changes.config ||
        unprovisionedClasses.length > 0 || retargetedFrom !== null;
    ...
    const buildId = formatBuildId(currentDate, generateUniqueId());
    ...
    const artifact = deepFreeze(version.toJSON());
    ...
    return {
        ...
        undeclaredSecretNames,
        artifact,
        deployOnCreate: deployment.deploy,
        preparedAt: currentDate,
        hashes: { modulesHash, bindingsHash, configHash },
        bindingsForHash: bindings,
        exportsForHash: exports,
    };
}

/**
 * Uploads one frozen prepared Worker artifact and records its identity.
 * @param {Object} args - Prepared creation inputs
 * @returns {Promise<CreateWorkerVersionResult>} Created version result
 */
export async function createPreparedWorkerVersion(args) {
    const {
        projectDirectory,
        environment,
        apiClient,
    ...
    const result = await apiClient.createWorkerVersion(
        prepared.workerName,
        prepared.artifact,
        { deploy: prepared.deployOnCreate, strictBindingsInheritance: true },
    );

    const bindings = prepared.bindingsForHash
        .map((binding) => {
            return binding.type === 'inherit'
                ? { ...binding, version_id: result.id }
                : binding;
        });
    ...
    const nextState = {
        ...
        versionId: result.id,
        createdAt: prepared.preparedAt.toISOString(),
        deployed: prepared.deployOnCreate,
        ...prepared.hashes,
        bindingsHash: hashValue({ bindings, exports: prepared.exportsForHash }),
    };
...
/**
 * Prepares and uploads a standalone undeployed Worker version.
 * @param {Object} args - Version inputs and dependencies
 * @returns {Promise<CreateWorkerVersionResult>} Version operation result
 */
export async function createWorkerVersion(args) {
    const prepared = await prepareWorkerVersion(args);

    if (prepared.outcome !== 'prepared') {
        return prepared;
    }

    return await createPreparedWorkerVersion({ ...args, prepared });
}
...
function validateSourceState(args) {
    const { state, workerName, environment, secretNames } = args;
    ...
    if (!state) {
        ...
        throw new UsageError(
            `No source Worker version is recorded for environment "${ environment }". ` +
            'Restore its .kixx Cloudflare state, or comment out every declaration, create a base version, ' +
            'then declare and set its secrets.',
        );
    }
...
// Strict inheritance would reject a missing secret at upload, but a release
// has staged content by then. Checking the exact source version first fails
// before any work, against Cloudflare rather than a local record that drifts.
function assertDeclaredSecretsSet(args) {
    const { sourceVersion, versionId, secretNames, environment } = args;
    const { missing, undeclared } = compareSecretNames({
        declaredNames: secretNames,
        remoteNames: readSecretBindingNames(sourceVersion),
    });

    if (missing.length > 0) {
        throw new UsageError(
            `Worker version ${ versionId } has no secret binding for declared ` +
            `secret${ missing.length === 1 ? '' : 's' }: ${ missing.join(', ') }. ` +
            `Set ${ missing.length === 1 ? 'it' : 'them' } with ` +
            `\`kixx.js cloudflare set-secrets -e ${ environment }\` before creating a Worker version.`,
        );
    }
...
async function verifySourceVersion(args) {
    const { apiClient, workerName, environment, versionId } = args;

    let version;
    try {
        version = await apiClient.getWorkerVersion(workerName, versionId);
    } catch (error) {
        if (error.status === 404) {
            throw new UsageError(
                `Recorded Worker version ${ versionId } does not exist for Worker "${ workerName }". ` +
                `Restore current state for environment "${ environment }" before creating a version.`,
                { cause: error },
            );
        }
    ...
    if (latest?.id !== versionId) {
        throw new UsageError(
            `Cloudflare state is stale for Worker "${ workerName }": recorded version ${ versionId }, ` +
            `latest remote version ${ latest?.id ?? 'none' }`,
        );
    }
 ...
 *
 * Only the release coordinator may allow the creation request to deploy,
 * because it stages content before invoking the prepared upload.
 */
function resolveDeployment(args) {
    const { allowForcedDeployment, unprovisionedClasses, workerName, bindings } = args;

    if (unprovisionedClasses.length === 0) {
        return { deploy: false, forced: false };
    }

    if (allowForcedDeployment) {
    ...
    throw new UsageError(
        ...
        'Use `kixx.js cloudflare release` so content is staged before Cloudflare can force deployment.',
    );
}
