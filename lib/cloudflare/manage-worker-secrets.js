...
export async function recoverSecretVersion(args) {
    ...
    const version = await getBaseVersion(apiClient, workerName, versionId);
    ...
    if (!isNonEmptyString(baseResources?.script?.etag) ||
        baseResources.script.etag !== resources?.script?.etag ||
        hashValue(baseResources.script) !== hashValue(resources.script) ||
        !isPlainObject(baseResources.script_runtime) || !isPlainObject(resources?.script_runtime) ||
        hashValue(baseResources.script_runtime) !== hashValue(resources.script_runtime) ||
        hashValue(baseVersion.exports ?? {}) !== hashValue(version.exports ?? {}) ||
        hashValue(baseVersion.cache_options ?? null) !== hashValue(version.cache_options ?? null)) {
        throw new UsageError('Cannot recover secret-only state: code or runtime differs, or verification data is missing');
    }
    const resourceSettings = (value) => Object.fromEntries(
        Object.entries(value).filter(([ name ]) => name !== 'bindings'),
    );
    if (hashValue(resourceSettings(baseResources)) !== hashValue(resourceSettings(resources))) {
        throw new UsageError('Cannot recover secret-only state: remote resource settings differ');
    }
