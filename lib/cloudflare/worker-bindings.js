...
/**
 * Assembles one environment's configuration blocks, plain dotenv values, and
 * declared inherited secrets into a validated, deterministically ordered array of Cloudflare
 * binding definitions. Every way of getting bindings wrong is caught here,
 * naming the config path or the dotenv key at fault.
 *
 * `.env.<environment>` values bind as `plain_text`. Secret names are inherited
 * from one exact Worker version; secret values never enter this module.
 *
 ...
 * @param {string[]} args.secretNames - Names declared in `example.env.secrets`.
 * @param {string} args.secretVersionId - Exact Worker version from which secrets are inherited.
 ...
 *     the dotenv file declares `BUILD_ID`, or a binding name collides across
 *     two sources.
 */
export function buildWorkerBindings(args) {
    const { environmentConfig, environment, envars, secretNames, secretVersionId } = args ?? {};
    ...
    const entries = [
        ...
        ...secretBindings(secretNames ?? [], secretVersionId),
    ];
...
function secretBindings(secretNames, versionId) {
    return secretNames.map((name) => ({
        source: 'example.env.secrets',
        binding: { type: 'inherit', name, version_id: versionId },
    }));
}
