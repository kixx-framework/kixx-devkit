...
// Cloudflare answers a rejected token and a token lacking a permission with
// the same generic error code (10000), so the status is the only signal a
// failed request gives us that the token is worth verifying.
const AUTH_STATUSES = [ 401, 403 ];

// The only token status Cloudflare reports for a token that still works.
// A token may also verify as "disabled" or "expired".
const ACTIVE_TOKEN_STATUS = 'active';
const SECRET_OPERATION_LIMIT = 100;

/**
 * @typedef {Object} TokenVerification
 * @property {string} status - `valid` when Cloudflare accepts the token,
 *     `invalid` when it rejects it, and `unknown` when Cloudflare could not
 *     be asked or gave an answer unrelated to the token.
 * @property {?string} detail - Cloudflare's own token status, such as
 *     `active`, `disabled`, or `expired`, when it reports one.
 * @property {CloudflareApiErrorDetail[]} errors - Cloudflare error array from
 *     the verification response, empty when there was none.
 */
...
export default class CloudflareAPIClient {
    ...
    /**
     * Asks Cloudflare whether the configured API token is usable at all.
     *
     * Nothing about a token can be judged locally: Cloudflare publishes no
     * format for them, so a well-formed string may still be revoked and a
     * future format may look nothing like today's. This is the only way to
     * separate "the token is dead" from "the token works but is not allowed
     * to do this", which are different problems for the user to fix.
     *
     * Reports a verdict instead of throwing, because its whole purpose is to
     * explain a failure that has already happened. An unreachable or
     * unhelpful Cloudflare yields `unknown` rather than an error which would
     * mask the original one.
     *
     * @returns {Promise<TokenVerification>} What Cloudflare says about the token
     */
    async verifyToken() {
        const url = new URL(`${ BASE_URL }/user/tokens/verify`);
        let res;
        ...
        try {
            ...
            return { status: 'unknown', detail: null, errors: [] };
        }

        const body = await readJsonBody(res);

        if (res.ok && body?.success) {
            const detail = body.result?.status ?? null;
            const isUsable = detail === null || detail === ACTIVE_TOKEN_STATUS;

            return {
                status: isUsable ? 'valid' : 'invalid',
                detail,
                errors: [],
            };
        }
        ...
        // Only an auth status means Cloudflare looked at the token and
        // refused it. A 5xx or a proxy error page says nothing about it.
        if (AUTH_STATUSES.includes(res.status)) {
            return { status: 'invalid', detail: null, errors };
        }
     ...
     * @param {Object} [options] - Additional version data to retrieve
     * @param {string} [options.include] - Set to 'modules' to include remote module contents
     * @returns {Promise<Object>} Unwrapped Worker version result
     * @throws {CloudflareApiError} When Cloudflare rejects the request or returns an unsuccessful API envelope
     */
    async getWorkerVersion(workerId, versionId, options) {
        ...
        assert(isUndefined(options) || isPlainObject(options), 'getWorkerVersion() options must be an object');
        if (!isUndefined(options?.include)) {
            assert(options.include === 'modules', 'getWorkerVersion() options.include must be modules');
     ...
     * @param {boolean} [options.strictBindingsInheritance] - Reject unresolved inherited bindings
    ...
    async createWorkerVersion(workerName, version, options) {
        ...
        assert(
            isUndefined(options?.strictBindingsInheritance) || isBoolean(options.strictBindingsInheritance),
            'CloudflareAPIClient#createWorkerVersion() options.strictBindingsInheritance must be a boolean',
        );
        ...
        if (options?.strictBindingsInheritance) {
    ...
    /**
     * Creates one undeployed Worker version containing an atomic secret patch.
     * Omitted names remain unchanged and null values delete their named secret.
     *
     * @see https://github.com/cloudflare/workers-sdk/blob/main/packages/wrangler/src/versions/secrets/index.ts
     *
     * @param {string} workerName - Worker name
     * @param {Object<string, string|null>} operations - Secret values or explicit deletions by name
     * @param {Object} options - Operation metadata
     * @param {string} options.command - Devkit command recorded on the created version
     * @returns {Promise<Object>} Created version identity with `versionId` and `createdAt`
     * @throws {CloudflareApiError} When Cloudflare rejects the mutation or returns no created version ID
     * @throws {Error} When metadata lookup fails after creation; the error preserves the created version ID
     */
    async createWorkerSecretVersion(workerName, operations, options) {
        assertNonEmptyString(
            workerName,
            'CloudflareAPIClient#createWorkerSecretVersion() requires a workerName',
        );
        assert(
            isPlainObject(operations),
            'CloudflareAPIClient#createWorkerSecretVersion() requires an operations object',
        );
        assert(
            isPlainObject(options),
            'CloudflareAPIClient#createWorkerSecretVersion() requires an options object',
        );
        assertNonEmptyString(
            options.command,
            'CloudflareAPIClient#createWorkerSecretVersion() requires options.command',
        );

        const entries = Object.entries(operations);
        assert(entries.length > 0, 'CloudflareAPIClient#createWorkerSecretVersion() requires an operation');
        assert(
            entries.length <= SECRET_OPERATION_LIMIT,
            `CloudflareAPIClient#createWorkerSecretVersion() accepts at most ${ SECRET_OPERATION_LIMIT } operations`,
        );
        ...
        entries.forEach(([ name, value ]) => {
            assertNonEmptyString(
                name,
                'CloudflareAPIClient#createWorkerSecretVersion() operation names must not be empty',
            );
            assert(
                value === null || isString(value),
                `CloudflareAPIClient#createWorkerSecretVersion() operation "${ name }" must be a string or null`,
            );
            ...
            secrets[name] = { type: 'secret_text', text: value };
        });

        const operationTag = `config-change-${ randomUUID() }`;
        const url = new URL(
            `${ BASE_URL }/accounts/${ this.#accountId }/workers/workers/${ workerName }/versions/latest`,
        );
        const method = 'PATCH';
        const headers = {
        ...
        const body = {
            ...
            annotations: {
                ...
                'workers/message': `Configuration change: ${ options.command }`,
                'workers/triggered_by': options.command,
            },
        };

        // The version PATCH endpoint returns the created ID directly; the
        // scripts secrets-bulk endpoint only returns secret metadata.
        const result = await this.#fetchResult(
            url,
            { method, headers, body: JSON.stringify(body) },
            { sensitiveValues },
        );
        if (!isString(result?.id) || !result.id) {
            throw new CloudflareApiError(
                `Cloudflare accepted the secret mutation but returned no version ID. ` +
                `Inspect tag "${ operationTag }" before retrying; local state was not updated.`,
                { status: 200, errors: [], method, url: url.href },
            );
        }

        // Preserve the causal ID even if the follow-up metadata read fails.
        try {
            const version = await this.getWorkerVersion(workerName, result.id);
            const createdAt = version.metadata?.created_on ?? version.created_on;
            assertNonEmptyString(createdAt, 'Cloudflare secret version response requires created_on');
            return { versionId: result.id, createdAt };
        } catch (cause) {
            throw new Error(
                `Created remote Worker version ${ result.id }, but could not read its metadata. ` +
                'Recover its local state with cloudflare recover-secret-version before another operation.',
                { cause },
            );
        }
    }
     ...
     * @throws {CloudflareApiError} When Cloudflare rejects the request or returns an unsuccessful API envelope
     */
    async #fetchResult(url, init, options) {
        const method = init.method || 'GET';
        const { sensitiveValues = [] } = options ?? {};
        const headers = {
        ...
        if (!res.ok) {
            ...
            const safeText = redactSensitiveValues(text, sensitiveValues);
            const details = safeText ? `: ${ safeText }` : '';

            // Spend one extra request, on the failure path only, to find out
            // which of the two problems an auth status hides. The caller
            // cannot tell the user whether to replace the token or widen its
            // permissions without knowing this.
            ...
            throw new CloudflareApiError(
                ...
                {
                    status: res.status,
                    errors: redactSensitiveValues(parseErrorEnvelope(text), sensitiveValues),
                    method,
                    url: url.href,
                    tokenStatus,
                },
        ...
        const safeMessage = redactSensitiveValues(message, sensitiveValues);
        const safeErrors = redactSensitiveValues(errors, sensitiveValues);
        throw new CloudflareApiError(
            `API error "${ safeMessage }" from ${ method } ${ url.href }`,
            { status: res.status, errors: safeErrors, method, url: url.href },
        );
    }
}
...
// Reads a response body which is expected to be a Cloudflare envelope but may
// be anything, including an HTML error page from a proxy in front of the API.
// Returns null rather than throwing, so an unreadable body is one more thing
// the caller knows nothing about instead of a second failure.
...
function redactSensitiveValues(value, sensitiveValues) {
    if (isString(value)) {
        return sensitiveValues.reduce((text, secret) => {
            return secret === '' ? text : text.replaceAll(secret, '[REDACTED]');
        }, value);
    }

    if (Array.isArray(value)) {
        return value.map((entry) => redactSensitiveValues(entry, sensitiveValues));
    }

    if (isPlainObject(value)) {
        const redacted = Object.create(null);
        Object.entries(value).forEach(([ key, entry ]) => {
            redacted[key] = redactSensitiveValues(entry, sensitiveValues);
        });
        return redacted;
    }
